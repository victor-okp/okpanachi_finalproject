...
water_raw %>%
  ...
  slice(1:10)   # show first 10 rows for clarity
...
# coliform by water source : This boxplot uses log-scaled coliform counts to compare contamination levels across water sources, and highlights variability and outliers in each category.
...
# coliform by Districts: This plot shows how coliform contamination varies across districts and allows visual comparison of central tendency, spread, and outliers.
...
# E.coli by water source: This boxplot displays E. coli levels across different water sources, using a log scale to make skewed microbial data easier to interpret.
...
# E.coli by Districts: This graph visualizes E. coli contamination differences across districts, highlighting variation in microbial levels and identifying potential outliers.
...
```{r}
# This section demonstrates spatial analysis techniques introduced in Week 9: Spatial Data, where we learned how to work with vector-based geographic data using the terra package. 
...
# Downloaded Tanzania’s administrative boundaries (Level 1 = regions/districts) from the GADM database. This provides the polygon layer for spatial context.
...
points(
  water_dar,
  col = cols_coliform[cut(water_dar$log_coliform, 30)],
  pch = 20,
  cex = 1.3
)
...
points(
  water_dar,
  col = cols_ecoli[cut(water_dar$log_ecoli, 30)],
  pch = 20,
  cex = 1.3
)
